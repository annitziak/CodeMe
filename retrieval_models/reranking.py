...
class Reranker:
    ...
    def rerank_metadata(self, retrieved_documents):
        ...
        metadata_df = self.metadata[
            self.metadata["id"].isin(retrieved_documents)
        ].copy()
        ...
        metadata_df["ranking_score"] = (
            ...
            (
                metadata_df["reputation_user"] * 1.5
            )  # Normalize reputation for fair scaling
            + (
                metadata_df["days_since_creation"] * 1.5
            )  # Boost newer documents - this actually might not matter this much
        )

        # Sort by ranking score (descending order)
        ranked_documents = metadata_df.sort_values(by="ranking_score", ascending=False)[
            "id"
        ].tolist()
